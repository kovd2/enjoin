...
<body>
	...
	<div class="container">
		...
		<p class="sideTitle">크루모집 게시판 관리자 페이지 입니다</p>
			...
			<tbody>
				...
				<c:forEach var="m" items="${map.list}" varStatus="status">
					<tr class="ContentBody">
						...
						<td
							onclick="crewRecruitmentDetails(${m.board_No },${m.user_No},${m.crew_No})">${m.board_Title}</td>
						<td>${m.crew_Area}</td>
						<td>${m.board_Count }</td>
						<td>${m.user_Id }</td>
						<td>${m.enroll_Date }</td>
			...
			<div class="pagination1">
				<nav>
					<ul class="pagination" style="display: inline-flex;">
						...
						<c:if test="${ pi.currentPage < pi.maxPage }">
							<c:url var="listEnd" value="adminCrewRecruitment.shw">
								<c:param name="currentPage" value="${ pi.currentPage + 1 }" />
							</c:url>
