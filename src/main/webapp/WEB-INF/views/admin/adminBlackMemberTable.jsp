				...
				<c:forEach var="m" items="${map.list}" varStatus="status">
					<tr class="ContentBody">
						...
						<td><button class="btn btn-outline-primary"
								onclick="goStatus(${m.userNo},'${m.userId}')">복원</button></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
			...
			<div class="pagination1">
				<nav>
					<ul class="pagination" style="display: inline-flex;">
						...
						<c:if test="${ pi.currentPage > 1 }">
							<c:url var="listBack" value="adminBlackMember.shw">
						...
						<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
							...
							<c:if test="${ p ne pi.currentPage }">
								<c:url var="listCheck" value="adminBlackMember.shw">
						...
						<c:if test="${ pi.currentPage < pi.maxPage }">
							<c:url var="listEnd" value="adminBlackMember.shw">
								<c:param name="currentPage" value="${ pi.currentPage + 1 }" />
							</c:url>
			...
			<tbody>
				<c:forEach var="m1" items="${map.list1}" varStatus="status">
					<tr class="ContentBody">
						...
						<td>${m1.enrollDate }</td>
						<td><button class="btn btn-outline-primary"
								onclick="copStatusMember(${m1.userNo},'${m1.userId}')">복원</button></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
